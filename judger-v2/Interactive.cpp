        ...
        if ((vpid = fork()) == 0) {
            ...
            if ((pid=fork())==0) {
                ...
                while (1) {
                    ...
                    if (WIFSIGNALED(runstat)&&WTERMSIG(runstat)!=SIGTRAP&&WTERMSIG(runstat)!=SIGPIPE) {
                    ...
                    else if (WIFSTOPPED(runstat)&&WSTOPSIG(runstat)!=SIGTRAP&&WSTOPSIG(runstat)!=SIGPIPE) {
                    ...
                    else if ((runstat>>8)!=SIGTRAP&&(runstat>>8)!=SIGPIPE&&(runstat>>8)>0) {
                        LOG((string)"Used time: "+Inttostring(time_used));
                        LOG((string)"Used Memory: "+Inttostring(memory_used));
                        LOG((string)"Run status: "+Inttostring(runstat));
            ...
            if ((pvid = fork()) == 0) {
                ...
                switch (language) {
                    ...
                    case ADALANG:
                        ...
                        break;
                    case JAVALANG:
                        execl("/usr/bin/java","java","-Djava.security.manager","-Djava.security.policy=java.policy","-client","-cp",CONFIG->GetTmpfile_path().c_str(),"Validator",NULL);
                        break;
                    case CSLANG:
                        execl("/usr/bin/mono","mono",validator_exc_filename.c_str(),NULL);
                        break;
                    case PERLLANG:
                        execl("/usr/bin/perl","perl",validator_src_filename.c_str(),"-W",NULL);
                    case RUBYLANG:
                        execl("/usr/bin/ruby","ruby",validator_src_filename.c_str(),"-W",NULL);
                        break;
                    case PYLANG:
                        execl("/usr/bin/python","python",validator_exc_filename.c_str(),NULL);
                        break;
                }
                exit(0);
            } else {
