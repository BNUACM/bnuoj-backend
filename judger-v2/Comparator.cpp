...
int Comparator::Compare() {
    ...
    if (isspj==0) {
        ...
        while (1) {
            ...
            if (so_char!=po_char) {
                detail=(string)"Expected "+so_char+", Found "+po_char+" Not AC.";
                LOG((string)"Expected "+so_char+", Found "+po_char+" Not AC.");
                aced=false;
                break;
            }
        }
        ...
        if (!aced) {
            ...
            while (1) {
                ...
                if (eofs==EOF||eofp==EOF) break;
                if (so_char!=po_char) {
                    detail+=(string)"\nExpected "+so_char+", Found "+po_char+" Not PE.";
                    LOG((string)"Expected "+so_char+", Found "+po_char+" Not PE.");
                    peed=false;
                    break;
                }
            }
        ...
        if (spj==NULL) {
            spj=new Program;
            if (isspj==1) {
                LOG((string)"Do SPJ, using "+"spj/"+spj->Inttostring(pid)+".cpp");
                if (!spj->Checkfile("spj/"+spj->Inttostring(pid)+".cpp")) {
                    LOG("No SPJ.");
                    return JE_STATUS;
                }
                spj->Setsource(spj->Loadallfromfile("spj/"+spj->Inttostring(pid)+".cpp"));
                spj->Setlanguage(CPPLANG);
            }
            else if (isspj==2) {
                LOG((string)"Do SPJ, using "+"spj/"+spj->Inttostring(pid)+".java");
                if (!spj->Checkfile("spj/"+spj->Inttostring(pid)+".java")) {
                    LOG("No SPJ.");
                    return JE_STATUS;
                }
                spj->Setsource(spj->Loadallfromfile("spj/"+spj->Inttostring(pid)+".java"));
