...
/**
 * Judger handler
 * @param arg   should be (JudgerArgs *), passes the socket handler and the oj name
 * @return NULL
 */
...
void start_listener() {
    ...
    while (true) {
        if ((client_fd = accept(main_sockfd, (struct sockaddr *) &remote_addr, &sin_size)) == -1) {
            throw Exception("Error in accept socket");
        }
        
        LOG((string)"Received a connection from " + inet_ntoa(remote_addr.sin_addr) + ":" + intToString(remote_addr.sin_port));
        pthread_t thread_id;
        int * arg = new int;
        *arg = client_fd;
